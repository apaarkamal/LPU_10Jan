...
vector<int> dijkstra(int source, int n) {
	...
	while (!Q.empty()) {
		...
		if (min_dis[cur] != INT_MAX) continue;
		// db(cur, path);
		// visiting first time update the shortest path
		min_dis[cur] = path;
		// add all new discovered esges with their paths into
		// the heap
		for (auto adj : gr[cur]) {
			Q.push({path + adj.second, adj.first});
		}
	}
	return min_dis;
}
...
int32_t main()
{
	...
	vector<int> shortest_path = dijkstra(0, n);

	for (int i = 0; i < n; i++) {
		cout << i << " " << shortest_path[i] << '\n';
	}
